...
- **Integration test execution belongs to the `ci-runner` agent**: you may WRITE integration tests, but do not run the IntegratedTests suite yourself in a long Docker/Testcontainers loop. Validate with build + unit tests, then tell the caller that integrated execution should be delegated to `ci-runner`.
- **Concurrency-sensitive gaps are scenarios, not code**: coverage gaps that require deep concurrency reasoning (graceful shutdown timing, race conditions, drain semantics in `src/Oragon.RabbitMQ/Consumer/`) must be reported as described scenarios for the `concurrency-specialist` agent — do not implement such tests blindly.
